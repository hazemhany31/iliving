...
class MockMaintenanceRepo implements MaintenanceRepository {
  ...
  @override
  Stream<List<MaintenanceRequest>> streamTicketsForUser(String userId) => Stream.value([]);

  @override
  Stream<List<MaintenanceRequest>> streamTicketsForCompound(String compoundId) => Stream.value([]);
...
void main() {
  ...
  testWidgets('Artifact 07: Admin Maintenance Hub', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1170, 2532);
    tester.view.devicePixelRatio = 3.0;
    addTearDown(tester.view.resetPhysicalSize);
    addTearDown(tester.view.resetDevicePixelRatio);
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: Scaffold(
          body: RepaintBoundary(
            key: const Key('capture_boundary'),
            child: MaintenanceModuleScreen(maintRepo: adminMockRepo),
          ),
        ),
      ),
    );
    ...
    await saveScreenshot(tester, '$brainDir/07_maintenance_valid_submission_admin.png');
    await tester.pumpWidget(const SizedBox());
  });
}
